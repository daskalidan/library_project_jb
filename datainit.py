...
def initialyze_database():
    ...
    try:
        ...
        init_books = [('the first book', 'me', '3', 'loaned'), ('bible', 'GOD', '1', 'in library'), ('the first book', 'me', '3', 'in library'),
    ...
    try:
        ...
        init_loan = [('000000001', 1, str(date.today()), str(date.today() + timedelta(days=2)), 'open')]

        for loan in init_loan:
                cur.execute(f'INSERT INTO loans (id_number, cat_number, loan_date, date_to_return, loan_status) VALUES {loan}')
        con.commit()    
